...
- 목적: HTML 특수문자를 엔티티로 변환하여 보안 강화
- 기능:
  < → &lt;    (HTML 태그 시작 방지)
  > → &gt;    (HTML 태그 종료 방지)
  " → &quot;  (HTML 속성 변조 방지)
  & → &amp;   (엔티티 문자 변환)
- 사용: 사용자 입력을 출력하거나 HTML 속성에 넣을 때

예시:
입력: <script>alert('공격')</script>
처리: htmlspecialchars() 적용
출력: &lt;script&gt;alert('공격')&lt;/script&gt;
결과: 스크립트 실행 안 됨 (안전!)
```
...
✅ **로그인 확인**
- 보안이 필요한 페이지는 $_SESSION['user_id'] 확인
- 미확인 사용자는 로그인 페이지로 리다이렉트
