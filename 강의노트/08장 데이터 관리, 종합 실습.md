...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // password_verify(입력값, 저장된해시) → true 또는 false
...
✅ **사용자 조회:** `SELECT id, username, password FROM users WHERE username = ?`
✅ **비밀번호 검증:** `password_verify($password_input, $user['password'])` (bcrypt 해시 검증)
✅ **세션 저장:** `$_SESSION['user_id']`, `$_SESSION['username']` 설정
✅ **리다이렉트:** 로그인 성공 시 `list_todos.php`로 이동
✅ **이미 로그인:** 로그인 상태면 `list_todos.php`로 즉시 이동
...
-- 테스트 데이터
INSERT INTO users (username, password) VALUES ('admin', '$2y$10$8oRplMLSjwW1lS7E8F0w.eDi1rCh7zqpSGtP3pPxWWIHVMIxm1C.m');
...
```
1. 선택한 프로젝트(TODO 또는 상품) 완성하기
   
2. 다음 기능이 모두 동작하는지 확인:
   - 추가하기 ✅
   - 목록 보기 ✅
   - 수정하기 ✅
   - 삭제하기 ✅
...
모든 UPDATE, DELETE 쿼리는 반드시 WHERE 절이 필요!
```
...
```php
prepare($sql)      // SQL 쿼리를 미리 준비
execute($params)   // 준비된 쿼리 실행
fetch()           // 하나의 행 반환
fetchAll()        // 모든 행 반환
```
...
1. **WHERE 절 확인**: UPDATE, DELETE 실행 전 반드시 WHERE 절 확인
2. **입력값 검증**: 사용자 입력은 항상 검증 필수
3. **htmlspecialchars()**: 데이터 출력 시 항상 사용
4. **에러 처리**: try-catch로 모든 쿼리 감싸기
5. **테스트**: 삭제 전에 항상 확인 메시지 표시
