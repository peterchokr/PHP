# 8장: 데이터 관리 & 종합 실습 (프로젝트)

---

## 학습목표

**학습목표**: 지난 7장까지 배운 내용을 통합하여 기본적인 웹 애플리케이션을 개발할 수 있음
...
# 프로젝트 1️⃣: TODO 관리 시스템

## 1️⃣ 프로젝트 개요

**목적**: 로그인한 사용자가 자신의 할 일(TODO)을 추가, 수정, 삭제, 완료 처리할 수 있는 시스템
...
├────┼─────────┼──────────┼──────────┼────────────┤
│ 1  │ 1       │ 장보기   │ incomplete│ 2024-01-11 │
│ 2  │ 1       │ 공부하기 │ complete  │ 2024-01-11 │
└────┴─────────┴──────────┴──────────┴────────────┘
...
### 2-3 테스트 데이터 생성 (setup_users.php)

테이블을 생성한 후, 다음 PHP 스크립트를 브라우저에서 실행하여 테스트 데이터를 생성하세요.
...
1. 위 SQL로 데이터베이스 및 테이블 생성
2. `setup_users.php` 파일을 서버에 업로드
3. 브라우저에서 `http://localhost/setup_users.php` 접속
4. ✅ 데이터 생성 완료 메시지 확인
...
- toggle_todo.php 파일 생성
- list_todos.php의 각 TODO 항목에 상태 변경 버튼 추가
- POST 요청으로 toggle_todo.php에 TODO id 전송
- 현재 상태를 조회한 후 반대 상태로 UPDATE
- UPDATE 완료 후 list_todos.php로 리다이렉트
...
- delete_todo.php 파일 생성
- list_todos.php의 각 TODO 항목에 삭제 버튼 추가
- JavaScript confirm() 함수로 사용자에게 확인 메시지 표시
- GET 요청으로 delete_todo.php에 TODO id 전송
- DELETE 쿼리로 데이터 삭제 (WHERE 절에 id와 user_id 필수)
- 삭제 완료 후 list_todos.php로 리다이렉트

### 4-3 페이지네이션 (선택사항)

**목표**: 1페이지에 5개의 TODO만 표시

**해야할 일**:

- list_todos.php의 SELECT 쿼리에 LIMIT과 OFFSET 추가
- 페이지 번호를 URL 파라미터로 받기 (?page=1)
- COUNT(*)로 전체 TODO 개수 조회
- 전체 페이지 수 계산: ceil(총 개수 / 5)
- 페이지 바닥에 "이전" "다음" 버튼 추가
...
**목적**: 관리자가 상품 정보를 추가, 수정, 삭제, 검색할 수 있는 시스템
...
-- 테스트 데이터는 아래의 setup_products.php로 생성합니다
```
...
├────┼──────────┼────────┼───────┼────────────┤
│ 1  │ 노트북   │1200000│ 10    │ 2024-01-11 │
│ 2  │ 마우스   │ 50000 │ 100   │ 2024-01-11 │
└────┴──────────┴────────┴───────┴────────────┘

- users: 관리자 계정 저장 (bcrypt 해싱)
- products: 관리자가 추가한 상품 정보 저장
```

---

### 2-3 테스트 데이터 생성 (setup_products.php)

테이블을 생성한 후, 다음 PHP 스크립트를 브라우저에서 실행하여 테스트 데이터를 생성하세요.

**파일명: `setup_products.php`** (1회만 실행)
...
try {
    ...
    $sql = "INSERT INTO users (username, password) VALUES (?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array('admin', $hashed));
  
    echo "✅ Admin (비밀번호: admin123) 사용자 생성 완료<br>";
    ...
    $products = array(
        array('name' => '노트북', 'price' => 1200000, 'stock' => 10),
        array('name' => '마우스', 'price' => 50000, 'stock' => 100)
    );
...
1. 위 SQL로 데이터베이스 및 테이블 생성
2. `setup_products.php` 파일을 서버에 업로드
3. 브라우저에서 `http://localhost/setup_products.php` 접속
4. ✅ 데이터 생성 완료 메시지 확인
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // 검증
        if (empty($name)) throw new Exception("상품명을 입력하세요");
        if (!is_numeric($price) || $price < 0) throw new Exception("가격을 올바르게 입력하세요");
        if (!is_numeric($stock) || $stock < 0) throw new Exception("재고를 올바르게 입력하세요");
...
<?php if ($success): ?>
    <div class="success"><?php echo htmlspecialchars($success); ?></div>
    <a href="list_products.php">목록으로 돌아가기</a>
<?php else: ?>
    <form method="POST">
...
<head>
    <title>상품 목록</title>
    <style>
        body { font-family: '맑은 고딕'; max-width: 1000px; margin: 50px auto; padding: 20px; }
        h1 { color: navy; }
        a { display: inline-block; margin: 10px 5px 10px 0; padding: 8px 12px; background: navy; color: white; text-decoration: none; }
...
<a href="add_product.php">➕ 새 상품 추가</a>
<a href="logout.php">🚪 로그아웃</a>

<?php if (empty($products)): ?>
    <p style="color: #999;">등록된 상품이 없습니다.</p>
<?php else: ?>
    <table>
        <tr>
            <th>상품명</th>
        ...
        <?php foreach ($products as $product): ?>
            <tr>
                ...
                <td class="action">
                    <a href="edit_product.php?id=<?php echo $product['id']; ?>">수정</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
...
**목표**: 상품을 삭제하고 목록으로 돌아가기

**해야할 일**:

- delete_product.php 파일 생성
- list_products.php의 각 상품 "작업" 열에 삭제 버튼 추가
- JavaScript confirm() 함수로 삭제 확인 메시지 표시
- GET 요청으로 delete_product.php에 상품 id 전송
- DELETE 쿼리로 데이터베이스에서 해당 상품 삭제
- 삭제 완료 후 list_products.php로 리다이렉트
...
- search_products.php 파일 생성
- HTML 폼에서 검색어를 입력받기 (GET 방식)
- LIKE 연산자를 사용하여 'name LIKE %keyword%' 형식으로 검색
- htmlspecialchars()로 검색 키워드 안전 처리
- 검색 결과를 테이블 형식으로 출력
- list_products.php에 검색 페이지로 이동하는 링크 추가
...
- products 테이블에 image 컬럼 추가 (VARCHAR 200)
- add_product.php와 edit_product.php에서 이미지 파일 input 추가
- 7장의 파일 업로드 검증 로직 적용 (파일 크기, 확장자 확인)
- 이미지 파일을 지정된 디렉토리에 저장
- list_products.php에서 상품 목록 보기 시 이미지 썸네일 표시
